### UX를 최우선으로 하는 개발자 김채정입니다 🙋‍♀

- ChungAng Univ. Physics. Cultural Contents.
- Web Front-end Developer
- 🔭 I’m currently working on B2B SaaS platform
- 🌱 I’m currently learning AI Agent, Tanstack Query
- 🤔 I’m looking for help with future career on Front-end
- 

<div>
  <a href="https://velog.io/@blcklamb">
    <img src="https://img.shields.io/badge/velog-20C997?style=flat&logo=velog&logoColor=white"/>
  </a>
</div>

### 📝 이력
- 22.07 ~ 22.10 2022 오픈소스 컨트리뷰션 아카데미 | githru-vscode-ext
- 22.11 ~ 23.04 클래스유 | Front-end 개발자
- 24.03 ~ 24.11 데브게이트 | TypeScript 개발자
- 25.01 ~ (재직 중) 로그블랙 | Front-end 개발자 

### 📈 교육
- 21.10 ~ 21.12 스파르타 | 비개발자를 위한, 웹개발 종합반(프로그래밍 실무, 풀스택)
- 22.01 ~ 22.07 엘리스 | AI 트랙 4기
- 22.08 ~ 22.09 원티드 | 프리온보딩 프론트엔드 코스 6차
- 23.09 ~ 23.12 남부여성발전센터 | 클라우드 웹개발 실무 프로젝트 

### 💝 수상
- 2022 엘리스 AI 트랙 4기 인공지능 웹 프로젝트 최우수상 "손생님"
- 2022 오픈소스 컨트리뷰션 우수상 "githru"
- 2025 관광데이터 활용 공모전 장려상 "경상북도 생태관광 인증 플랫폼"

